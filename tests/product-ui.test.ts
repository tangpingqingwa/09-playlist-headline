...
test("rules empty-playback names last-7-days — not this week", () => {
  const rules = renderToStaticMarkup(createElement(RulesPage));
  const emptyPlaybackAt = rules.indexOf(
    "If last 7 days has no paid #1, there is no player and no opening song",
  );
  const minBidAt = rules.indexOf("First bid for a listing last 7 days must be");
  const fakeStreamsAt = rules.indexOf("No fake streams");
  assert.notEqual(emptyPlaybackAt, -1);
  assert.notEqual(minBidAt, -1);
  assert.notEqual(fakeStreamsAt, -1);
  assert.ok(minBidAt < emptyPlaybackAt);
  assert.ok(fakeStreamsAt < emptyPlaybackAt);
  assert.match(
    rules,
    /If last 7 days has no paid #1, there is no player and no opening song/,
  );
  ...
  assert.doesNotMatch(rules, /Hear this week/);
  assert.doesNotMatch(rules, /data-first-click="hear"/);
  assert.doesNotMatch(rules, /hear-after-need-N/);
  assert.match(rules, /data-page="rules"/);
  assert.match(rules, /First bid for a listing last 7 days must be/);
  assert.match(rules, /<strong>\$5<\/strong>/);
  assert.match(rules, /Older wins ties/);
  ...
  assert.doesNotMatch(rulesSource, /If the week has no paid #1/);
  assert.doesNotMatch(rulesSource, /Hear last 7 days/);
  assert.doesNotMatch(rulesSource, /Hear this week/);
  assert.match(
    specSource,
  ...
  const specEmptyPlaybackCopyAt = specSource.indexOf(
    "SPEC empty-playback copy names last 7 days, not this calendar week",
  );
  const rulesEmptyPlaybackCopyAt = specSource.indexOf(
    "Rules empty-playback copy names last 7 days, not this calendar week",
  );
  assert.notEqual(specEmptyPlaybackCopyAt, -1);
  assert.notEqual(rulesEmptyPlaybackCopyAt, -1);
  assert.ok(specEmptyPlaybackCopyAt < rulesEmptyPlaybackCopyAt);
  assert.match(specSource, /Empty stays Claim #1 first, no Hear/);
  assert.doesNotMatch(specSource, /If the week has no paid #1/);
  assert.doesNotMatch(specSource, /Hear last 7 days/);
  assert.doesNotMatch(specSource, /Hear this week/);
  ...
  assert.match(formSource, /Claim #1 for/);
  assert.doesNotMatch(formSource, /Hear last 7 days/);
  assert.match(cssSource, /Rules empty-playback names last 7 days/);
  assert.match(cssSource, /SPEC empty-playback names last 7 days/);
  assert.match(cssSource, /SPEC persona names last 7 days/);
  assert.match(cssSource, /README weekly names last 7 days/);
  assert.match(cssSource, /About weekly names last 7 days/);
  ...
  assert.match(cssSource, /Empty site metadata names last 7 days/);
  assert.match(cssSource, /Empty lede names last 7 days/);
  const rulesEmptyPlaybackCommentAt = cssSource.indexOf(
    "Rules empty-playback names last 7 days",
  );
  const specEmptyPlaybackCommentAt = cssSource.indexOf(
    "SPEC empty-playback names last 7 days",
  );
  const personaCommentAt = cssSource.indexOf("SPEC persona names last 7 days");
  const readmeCommentAt = cssSource.indexOf("README weekly names last 7 days");
  const aboutCommentAt = cssSource.indexOf("About weekly names last 7 days");
  const rulesMinBidCommentAt = cssSource.indexOf(
    "Rules min-bid names last 7 days",
  );
  const pitchCommentAt = cssSource.indexOf(
    "Empty product pitch names last 7 days",
  ...
  assert.ok(rulesEmptyPlaybackCommentAt > specEmptyPlaybackCommentAt);
  assert.ok(specEmptyPlaybackCommentAt > personaCommentAt);
  assert.ok(personaCommentAt > readmeCommentAt);
  assert.ok(readmeCommentAt > aboutCommentAt);
  assert.ok(aboutCommentAt > rulesMinBidCommentAt);
  assert.ok(rulesMinBidCommentAt > pitchCommentAt);
  assert.ok(pitchCommentAt > metaCommentAt);
  const occupiedHearRulesEmptyPlaybackKeep =
    cssSource.match(
      /\.week-occupied \.opening-listen\[data-hear-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(occupiedHearRulesEmptyPlaybackKeep, /font-weight:\s*700/);
  const emptyLedeRulesEmptyPlaybackKeep =
    cssSource.match(
      /\.week-empty \.lede\[data-first-read="bid"\]\[data-empty-lede-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(emptyLedeRulesEmptyPlaybackKeep, /font-weight:\s*600/);
  const kickerRulesEmptyPlaybackKeep =
    cssSource.match(
      /\.week-empty \.empty-deck \.deck-kicker\[data-empty-kicker\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(kickerRulesEmptyPlaybackKeep, /font-weight:\s*600/);
  const claimRulesEmptyPlaybackKeep =
    cssSource.match(
      /\.week-empty \.claim\.empty-claim-first \.claim-note\[data-empty-claim-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(claimRulesEmptyPlaybackKeep, /font-weight:\s*600/);
  assert.doesNotMatch(pageSource, /24h lock/);
  assert.doesNotMatch(cssSource, /hear-after-need-six|need-after-hear-six/);
  assert.doesNotMatch(cssSource, /hear-after-need-N/);
});
...
test("rules empty-week names last-7-days — not this week", () => {
  const rules = renderToStaticMarkup(createElement(RulesPage));
  const emptyWeekAt = rules.indexOf("An empty last 7 days is valid");
  const emptyPlaybackAt = rules.indexOf(
    "If last 7 days has no paid #1, there is no player and no opening song",
  );
  const minBidAt = rules.indexOf("First bid for a listing last 7 days must be");
  const fakeStreamsAt = rules.indexOf("No fake streams");
  assert.notEqual(emptyWeekAt, -1);
  assert.notEqual(emptyPlaybackAt, -1);
  assert.notEqual(minBidAt, -1);
  assert.notEqual(fakeStreamsAt, -1);
  assert.ok(minBidAt < emptyWeekAt);
  assert.ok(emptyWeekAt < emptyPlaybackAt);
  assert.ok(emptyWeekAt < fakeStreamsAt);
  assert.match(rules, /An empty last 7 days is valid/);
  assert.doesNotMatch(rules, /An empty week is valid/);
  assert.doesNotMatch(rules, /Hear last 7 days/);
  assert.doesNotMatch(rules, /Hear this week/);
  assert.doesNotMatch(rules, /data-first-click="hear"/);
  assert.doesNotMatch(rules, /hear-after-need-N/);
  ...
  assert.match(rulesSource, /An empty last 7 days is valid/);
  assert.doesNotMatch(rulesSource, /An empty week is valid/);
  assert.doesNotMatch(rulesSource, /Hear last 7 days/);
  assert.doesNotMatch(rulesSource, /Hear this week/);
  assert.match(
    rulesSource,
  ...
  const rulesEmptyWeekCopyAt = specSource.indexOf(
    "Rules empty-week copy names last 7 days, not this calendar week",
  );
  assert.notEqual(specEmptyPlaybackCopyAt, -1);
  assert.notEqual(rulesEmptyPlaybackCopyAt, -1);
  assert.notEqual(rulesEmptyWeekCopyAt, -1);
  assert.ok(specEmptyPlaybackCopyAt < rulesEmptyPlaybackCopyAt);
  assert.ok(rulesEmptyPlaybackCopyAt < rulesEmptyWeekCopyAt);
  assert.match(specSource, /Empty stays Claim #1 first, no Hear/);
  assert.doesNotMatch(specSource, /If the week has no paid #1/);
  assert.doesNotMatch(specSource, /Hear last 7 days/);
  assert.doesNotMatch(specSource, /Hear this week/);
  ...
  assert.match(formSource, /Claim #1 for/);
  assert.doesNotMatch(formSource, /Hear last 7 days/);
  assert.match(cssSource, /Rules empty-week names last 7 days/);
  assert.match(cssSource, /Rules empty-playback names last 7 days/);
  assert.match(cssSource, /SPEC empty-playback names last 7 days/);
  assert.match(cssSource, /SPEC persona names last 7 days/);
  assert.match(cssSource, /README weekly names last 7 days/);
  ...
  assert.match(cssSource, /Empty site metadata names last 7 days/);
  assert.match(cssSource, /Empty lede names last 7 days/);
  const rulesEmptyWeekCommentAt = cssSource.indexOf(
    "Rules empty-week names last 7 days",
  );
  const rulesEmptyPlaybackCommentAt = cssSource.indexOf(
    "Rules empty-playback names last 7 days",
  );
  ...
  assert.ok(rulesEmptyWeekCommentAt > rulesEmptyPlaybackCommentAt);
  assert.ok(rulesEmptyPlaybackCommentAt > specEmptyPlaybackCommentAt);
  assert.ok(specEmptyPlaybackCommentAt > personaCommentAt);
  assert.ok(personaCommentAt > readmeCommentAt);
  assert.ok(readmeCommentAt > aboutCommentAt);
  ...
  assert.ok(rulesMinBidCommentAt > pitchCommentAt);
  assert.ok(pitchCommentAt > metaCommentAt);
  const occupiedHearRulesEmptyWeekKeep =
    cssSource.match(
      /\.week-occupied \.opening-listen\[data-hear-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(occupiedHearRulesEmptyWeekKeep, /font-weight:\s*700/);
  const emptyLedeRulesEmptyWeekKeep =
    cssSource.match(
      /\.week-empty \.lede\[data-first-read="bid"\]\[data-empty-lede-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(emptyLedeRulesEmptyWeekKeep, /font-weight:\s*600/);
  const kickerRulesEmptyWeekKeep =
    cssSource.match(
      /\.week-empty \.empty-deck \.deck-kicker\[data-empty-kicker\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(kickerRulesEmptyWeekKeep, /font-weight:\s*600/);
  const claimRulesEmptyWeekKeep =
    cssSource.match(
      /\.week-empty \.claim\.empty-claim-first \.claim-note\[data-empty-claim-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(claimRulesEmptyWeekKeep, /font-weight:\s*600/);
  assert.doesNotMatch(pageSource, /24h lock/);
  assert.doesNotMatch(cssSource, /hear-after-need-six|need-after-hear-six/);
  assert.doesNotMatch(cssSource, /hear-after-need-N/);
});
...
test("rules weekly-reset heading names last-7-days — not this week", () => {
  const rules = renderToStaticMarkup(createElement(RulesPage));
  const headingAt = rules.indexOf("<h2>Rolling last 7 days</h2>");
  const calendarHeadingAt = rules.indexOf("<h2>Weekly UTC reset</h2>");
  const emptyWeekAt = rules.indexOf("An empty last 7 days is valid");
  const emptyPlaybackAt = rules.indexOf(
    "If last 7 days has no paid #1, there is no player and no opening song",
  );
  ...
  assert.equal(calendarHeadingAt, -1);
  assert.notEqual(emptyWeekAt, -1);
  assert.notEqual(emptyPlaybackAt, -1);
  assert.notEqual(minBidAt, -1);
  assert.notEqual(fakeStreamsAt, -1);
  assert.ok(minBidAt < headingAt);
  assert.ok(headingAt < emptyWeekAt);
  assert.ok(emptyWeekAt < emptyPlaybackAt);
  assert.ok(emptyWeekAt < fakeStreamsAt);
  ...
  assert.match(rulesSource, /<h2>Rolling last 7 days<\/h2>/);
  assert.doesNotMatch(rulesSource, /<h2>Weekly UTC reset<\/h2>/);
  assert.match(rulesSource, /An empty last 7 days is valid/);
  assert.doesNotMatch(rulesSource, /An empty week is valid/);
  assert.doesNotMatch(rulesSource, /Hear last 7 days/);
  assert.doesNotMatch(rulesSource, /Hear this week/);
  ...
  const rulesWeeklyResetCopyAt = specSource.indexOf(
    "Rules weekly-reset heading names last 7 days, not this calendar week",
  );
  assert.notEqual(specEmptyPlaybackCopyAt, -1);
  assert.notEqual(rulesEmptyPlaybackCopyAt, -1);
  assert.notEqual(rulesEmptyWeekCopyAt, -1);
  assert.notEqual(rulesWeeklyResetCopyAt, -1);
  assert.ok(specEmptyPlaybackCopyAt < rulesEmptyPlaybackCopyAt);
  assert.ok(rulesEmptyPlaybackCopyAt < rulesEmptyWeekCopyAt);
  assert.ok(rulesEmptyWeekCopyAt < rulesWeeklyResetCopyAt);
  assert.match(specSource, /Empty stays Claim #1 first, no Hear/);
  assert.doesNotMatch(specSource, /If the week has no paid #1/);
  assert.doesNotMatch(specSource, /Hear last 7 days/);
  assert.doesNotMatch(specSource, /Hear this week/);
  ...
  assert.match(formSource, /Claim #1 for/);
  assert.doesNotMatch(formSource, /Hear last 7 days/);
  assert.match(cssSource, /Rules weekly-reset heading names last 7 days/);
  assert.match(cssSource, /Rules empty-week names last 7 days/);
  assert.match(cssSource, /Rules empty-playback names last 7 days/);
  assert.match(cssSource, /SPEC empty-playback names last 7 days/);
  assert.match(cssSource, /SPEC persona names last 7 days/);
  ...
  assert.match(cssSource, /Empty site metadata names last 7 days/);
  assert.match(cssSource, /Empty lede names last 7 days/);
  const rulesWeeklyResetCommentAt = cssSource.indexOf(
    "Rules weekly-reset heading names last 7 days",
  );
  const rulesEmptyWeekCommentAt = cssSource.indexOf(
    "Rules empty-week names last 7 days",
  );
  ...
  assert.ok(rulesWeeklyResetCommentAt > rulesEmptyWeekCommentAt);
  assert.ok(rulesEmptyWeekCommentAt > rulesEmptyPlaybackCommentAt);
  assert.ok(rulesEmptyPlaybackCommentAt > specEmptyPlaybackCommentAt);
  assert.ok(specEmptyPlaybackCommentAt > personaCommentAt);
  assert.ok(personaCommentAt > readmeCommentAt);
  ...
  assert.ok(rulesMinBidCommentAt > pitchCommentAt);
  assert.ok(pitchCommentAt > metaCommentAt);
  const occupiedHearRulesWeeklyResetKeep =
    cssSource.match(
      /\.week-occupied \.opening-listen\[data-hear-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(occupiedHearRulesWeeklyResetKeep, /font-weight:\s*700/);
  const emptyLedeRulesWeeklyResetKeep =
    cssSource.match(
      /\.week-empty \.lede\[data-first-read="bid"\]\[data-empty-lede-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(emptyLedeRulesWeeklyResetKeep, /font-weight:\s*600/);
  const kickerRulesWeeklyResetKeep =
    cssSource.match(
      /\.week-empty \.empty-deck \.deck-kicker\[data-empty-kicker\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(kickerRulesWeeklyResetKeep, /font-weight:\s*600/);
  const claimRulesWeeklyResetKeep =
    cssSource.match(
      /\.week-empty \.claim\.empty-claim-first \.claim-note\[data-empty-claim-window\]\s*\{[^}]+\}/,
    )?.[0] ?? "";
  assert.match(claimRulesWeeklyResetKeep, /font-weight:\s*600/);
  assert.doesNotMatch(pageSource, /24h lock/);
  assert.doesNotMatch(cssSource, /hear-after-need-six|need-after-hear-six/);
  assert.doesNotMatch(cssSource, /hear-after-need-N/);
});
